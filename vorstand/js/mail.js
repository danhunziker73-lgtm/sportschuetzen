        ...
        <div class="card border-0 shadow-sm">
          <div class="card-body">
            ...
            <div id="mail-mailto-hint" class="alert alert-warning py-2 small d-none">
              ...
              Zu viele Empfänger (&gt;30) – bitte
              <strong>«Adressen kopieren»</strong> und manuell ins BCC einfügen.
            </div>
...
function _getKatLabel(m) {
  ...
  if (gewählt('kk50a')    && m._istKK50A)    return m._kkKategorie  || 'KK 50m Aktiv-A';
  if (gewählt('kk50b')    && m._istKK50B)    return m._kkKategorie  || 'KK 50m Aktiv-B';

  // Fallback wenn mehrere Gruppen gewählt oder «Alle»
  if (m._istVorstand) return 'Vorstand';
  if (m._istEhren)    return 'Ehrenmitglied';
  if (m._istPassiv)   return 'Passiv';
  if (m._istKK50A)    return m._kkKategorie  || 'KK 50m Aktiv-A';
  if (m._istKK50B)    return m._kkKategorie  || 'KK 50m Aktiv-B';
...
function _mailGetTrenner() {
  ...
  // iOS / Android / Mac → Komma (Apple Mail, Outlook Mobile)
  // Windows           → Semikolon (Outlook Desktop)
...
function _mailRenderSummary() {
  ...
  const warnHtml = ohne > 0
    ? `<span class="text-warning ms-2">
         <i class="fas fa-exclamation-triangle"></i> ${ohne} ohne E-Mail
       </span>`
    : '';

  summaryEl.innerHTML =
    `<strong>${alle.length} Empfänger</strong>, ${mitEmail.length} mit E-Mail${warnHtml}`;

  // BCC-Hinweis mit erkanntem Trennzeichen
  const trenner     = _mailGetTrenner();
  const clientLabel = trenner === ',' ? 'iOS / Mac / Android' : 'Windows / Outlook';
  hintEl.innerHTML  =
    `<i class="fas fa-info-circle me-1"></i>
     Empfänger bitte ins <strong>BCC-Feld</strong> einfügen –
     so bleiben die Adressen der Mitglieder geschützt.
     <span class="text-muted ms-1">(Trennzeichen: <code>${trenner}</code> · ${clientLabel})</span>`;
  ...
  document.getElementById('mail-preview').innerHTML = `
    <table class="table table-sm table-hover mb-0">
      <thead class="table-light"
             style="position:sticky;top:0;z-index:1;cursor:pointer;user-select:none">
        <tr>
