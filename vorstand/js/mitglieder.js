...
function mglRenderStats(data) {
  ...
  const mitLizenz = data.filter(m => Number(m._aktiveLizenzenCount || 0) > 0).length;
  const ehren = data.filter(m => m._istEhren).length;
  const vorstand = data.filter(m => Number(m._aktiveFunktionenCount || 0) > 0).length;

  const el = document.getElementById('mglStats');
  if (!el) return;

  el.innerHTML = `
    <span class="badge bg-primary fs-6 px-3">👥 Total: ${total}</span>
    <span class="badge bg-success fs-6 px-3">🏅 Mit aktiver Lizenz: ${mitLizenz}</span>
    <span class="badge bg-warning text-dark fs-6 px-3">🏆 Ehrenmitglieder: ${ehren}</span>
    <span class="badge bg-info text-dark fs-6 px-3">⭐ Aktive Funktionen: ${vorstand}</span>`;
}
...
function mglRenderRows(data) {
  ...
  tbody.innerHTML = data.map(m => {
    ...
    const katBadge = mglKatBadge(m._kategorie || '');
...
function mglStatusBadge(m) {
  if (m._istEhren) return '<span class="badge bg-warning text-dark">Ehrenmitglied</span>';
  if (m._istPassiv) return '<span class="badge bg-secondary">Passiv</span>';
  if (m.IsActive == 1 || m.IsActive === true || m.IsActive === '1') {
...
function mglFilter() {
  ...
  let filtered = _mglData.filter(m => {
    ...
    const matchStatus =
      !status ||
      (status === 'aktiv' && isAktiv && !isPassiv && !isEhren) ||
      (status === 'passiv' && isPassiv) ||
      (status === 'inaktiv' && !isAktiv) ||
      (status === 'ehren' && isEhren);
...
async function mglOpenDetail(pn) {
  ...
  try {
    ...
    const badgeAktiv = (m._badgeAktiv || (m.IsActive == 1 || m.IsActive === true || m.IsActive === '1')) && !m._istPassiv;
    ...
    const fnRows = fn.map(f => {
      ...
      return `<tr>
        ...
        <td>${(f.rabatt_kategorie || f.rabattkategorie) ? `<span class="badge bg-warning text-dark">${f.rabatt_kategorie || f.rabattkategorie}</span>` : '–'}</td>
    ...
    const timeline = timelineData.length
      ? timelineData.map(t => `
        <div class="mgl-timeline-item">
          <div class="mgl-timeline-dot" style="background:${t.color || '#2563eb'}"></div>
          <div class="mgl-timeline-card">
            <div class="d-flex flex-wrap justify-content-between align-items-start gap-2">
              <div>
                <div class="mgl-timeline-title">${t.icon || '•'} ${t.title || t.typ || 'Eintrag'}</div>
                ${t.text ? `<div class="small mt-1">${t.text}</div>` : ''}
                ${t.meta ? `<div class="mgl-timeline-meta mt-1">${t.meta}</div>` : ''}
              </div>
        ...
        ? his.map(h => `
          <div class="mgl-timeline-item">
            ...
            <div class="mgl-timeline-card">
              <div class="d-flex flex-wrap justify-content-between align-items-start gap-2">
                <div>
                  <div class="mgl-timeline-title">${h.ereignis_typ || 'Historie'}</div>
                  ${(h.neuer_wert || h.alter_wert) ? `<div class="small mt-1">${h.neuer_wert || h.alter_wert}</div>` : ''}
                  ${h.erfasst_von ? `<div class="mgl-timeline-meta mt-1">${h.erfasst_von}</div>` : ''}
                </div>
          ...
          <div class="d-flex flex-wrap gap-3 align-items-center">
            ...
            <div class="flex-grow-1">
              ...
              <div class="mgl-badge-row">
                ${badgeAktiv ? `<span class="mgl-chip green">Aktiv</span>` : `<span class="mgl-chip gray">Inaktiv</span>`}
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <div class="row border-bottom py-1">
                  ...
                  <div class="col-7 small">${m.Niemahnen || m.Nie_mahnen ? '✅ Ja' : 'Nein'}</div>
                </div>
                <div class="row border-bottom py-1">
                  <div class="col-5 text-muted small">Vereinsaustritt</div>
                  <div class="col-7 small">${mglFmtDate(m.Vereinsaustritt || m.Vereins_austritt)}</div>
                </div>
                <div class="row py-1">
            ...
            <div class="row g-3 mt-1">
              ...
              <div class="col-md-4">
                ...
                <input type="date" class="form-control" id="mglEditAustritt"
                       value="${mglFmtDateIso(m.Vereinsaustritt || m.Vereins_austritt)}">
              </div>
              <div class="col-md-4">
                <div class="form-check mt-4">
                  <input type="checkbox" class="form-check-input" id="mglEditMahnen"
                         ${(m.Niemahnen || m.Nie_mahnen) ? 'checked' : ''}>
  ...
  try {
    ...
    if (idx >= 0) {
      Object.assign(_mglData[idx], {
        ...
        Vereinsaustritt: params.Vereins_austritt,
        Niemahnen: params.Nie_mahnen === '1'
      });
    }
