...
function loadResultateData() {
  ...
  apiFetch("manager", "action=getResultateData&sheetName=aktuell_Grenzland")
    ...
    .then(txt => {
      ...
      incoming.forEach(r => applyTeamInheritance(r, true));
...
function applyTeamInheritance(row, initial) {
  if (initial) {
    row._autoR2 = !row.r2_team;
    row._autoR3 = !row.r3_team;
  }
  if (!row.r2_team) { row.r2_team = row.r1_team || ""; row._autoR2 = true; }
  if (!row.r3_team) { row.r3_team = row.r2_team || row.r1_team || ""; row._autoR3 = true; }
}
